...
export default function JwtTokenInfo({ showInHeader = false, compact = false }: TokenInfoProps) {
  ...
  useEffect(() => {
    ...
    const updateTokenInfo = () => {
      ...
      if (!currentUser) {
        console.log('JWT Token Info: No current user found');
        setTimeRemaining(0);
        setIsExpiring(false);
        return;
      }
      ...
      if (!token) {
        console.log('JWT Token Info: No token found in localStorage');
        setTimeRemaining(0);
        setIsExpiring(false);
        return;
      }
      ...
      console.log('JWT Token Info:', { remaining, expiringSoon, user: currentUser });
